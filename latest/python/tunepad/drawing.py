...
def getCanvas(canvas_name = None):
    ...
    def strokeCircle(self, x, y, radius):
        self.beginPath()
        self.arc(x, y, radius, 0, pi * 2)
        self.stroke()

    ctx.strokeCircle = MethodType(strokeCircle, ctx)
...
# prompt the user to input text through a modal dialog
def prompt(message):
    return run_js(f'_asyncPrompt("{message}")')

def setDOMElementAttribute(selector, attributeName, attributeValue):
    return run_js(f'_setDOMElementAttribute("{selector}", "{attributeName}", "{attributeValue}")')
