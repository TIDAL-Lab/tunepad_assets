...
export declare class Synthesizer {
     ...
     * @param note note to be played (pitch and duration)
     * @param dest optional audio destination
     */
    playSound(note: Note, soundURL: string, dest?: AudioNode): void;
    /**
    ...
    scheduleSound(note: Note | number, soundURL: string, start: number, delta?: number, dest?: AudioNode): SynthChain | undefined;
    /**
     * When user drags the playhead over an audio buffer, use this to preview the sound
     */
    scrubAudio(buffer: AudioBuffer | undefined, start: number, end: number, dest?: AudioNode): void;
